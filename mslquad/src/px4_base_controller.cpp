...
PX4BaseController::PX4BaseController() : quadNS_("/"),
                                         fixedHeight_(0),
                                         maxVel_(0),
                                         controlLoopFreq_(20),
                                         slowLoopFreq_(10),
                                         state_(State::AUTO)
{
    ...
    joySub_ = nh_.subscribe(
        quadNS_ + "/joy",1, &PX4BaseController::pathCB, this);
    ...
    while (ros::ok() && !mocapCheck)
    {
        mocapCheck = true;
        for (int i = 0; i < 10; ++i)
        { // must be consistent for 10 checks
