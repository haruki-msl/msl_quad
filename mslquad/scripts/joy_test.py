...
class Pad(object):
    def __init__(self):
        ...
        #state info
        self.pose=Pose()
        self.agentPose_sub = rospy.Subscriber('mavros/local_position/pose', PoseStamped, self.poseCB)
    ...
    def modeSet(self, joy):
        ...
        modeVal= [self.buttonState(joy, 'mode_set1'), self.buttonState(joy, 'mode_set2')]
        if modeVal== [0, 1]:
            self.mode='POS'
